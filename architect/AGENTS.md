...
Rules:
- **Initial Context:** Begin by reading `../README.md`. Do not load `TASKS.md` or an existing `DESIGN.md` blindly; follow their measurement and bounded-reading procedures first. Inspect only the files under `../implementer/` that are relevant to understanding the language, architecture, test framework, or current task. Avoid loading unrelated implementation details into your context.
...
- **Task State Management:** You exclusively own `TASKS.md`. Keep it a bounded rolling execution queue, not an append-only task archive or implementation journal.
  1. **Measure Before Loading:** If `TASKS.md` exists, run `wc -w TASKS.md` and `wc -c TASKS.md` before reading it. It must not exceed 1,500 words or 10,000 bytes. If oversized, do not load it in full. Locate headings, unchecked items, active blockers, and recent completions with targeted searches; inspect ranges of at most 200 lines and 6,000 bytes; then rewrite it into the required rolling structure. Start a fresh Architect session before cumulative legacy excerpts exceed 10,000 bytes. Git preserves removed history. If the file is missing, create it with the required structure.
...
- **Architecture State Management:** You exclusively own `DESIGN.md` in this directory. Keep it a compact snapshot of the current architecture, not an append-only journal. Never ask an Implementer to modify any file under `../architect/`.
  1. **Initialize and Validate:** If `DESIGN.md` exists, measure it with `wc -w DESIGN.md` and `wc -c DESIGN.md` before loading it. If it is within budget, read it and validate its required structure and DRY rules. If it exceeds either limit, do not load it during a normal planning session and do not delegate implementation; report the counts and perform dedicated compaction first. During compaction, use ranged reads of at most 200 lines and 6,000 bytes each, compact the material in place, and start a fresh Architect session before cumulative legacy excerpts in the current context exceed 12,000 bytes. Do not accumulate copied excerpts in notes; Git preserves the original history. If `DESIGN.md` does not exist, create and populate it before delegating implementation.
